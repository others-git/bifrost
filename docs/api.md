...
| Method | Path | Auth | Purpose |
|---|---|---|---|
| `POST` | `/api/kiosks/checkin` | **kiosk key** | Heartbeat: `{ name?, app_version?, screen_on?, battery/power telemetry…, device_owner?, lock_task?, keyguard_disabled?, keyguard_locked? }` → `{ command, room, default_board_id }` (queued command, consumed; `room` is the assigned Room's name — the app's voice context) |
...
| `PUT` | `/api/kiosks/{id}/aware-override` | session | Aware-hour device override: `{ targets, mode? }` — `targets` is a `[{domain, id}]` list (light/media/power); while any is on, `mode` `keep_on` (default) holds the screen awake regardless of presence, `keep_off` forces it asleep. Empty `targets` clears |
