...
- **Boards.** Compose your own dashboards from widgets — room cards, device tiles, groups, buttons, now-playing (with album art), scenes, sensors, weather, clocks, recently-added media feeds (Plex) — on a fixed-aspect grid that renders identically on any screen. A board can be seeded from a room, run full-screen as a kiosk, and a paired wall tablet can auto-launch one (see [Boards](boards.md)).
