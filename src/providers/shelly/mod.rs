...
impl ShellyProvider {
    fn new_with_base(base_url: impl Into<String>) -> Result<Self> {
        // One pooled client shared across all Shelly devices (no per-device config;
        // it pools per-host internally), so per-request rebuilds reuse warm
        // connections. See [`crate::providers::cached_client`].
        let client = crate::providers::cached_client("shelly", || {
