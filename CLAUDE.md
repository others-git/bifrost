...
- **Cutting a release? Bump `version` in `Cargo.toml` to match the release tag / `Release X.Y.Z` commit — in the same commit.** `Cargo.toml`'s version is the *single source* of the in-app version: `env!("CARGO_PKG_VERSION")` → the `/api` health endpoint (`src/api/mod.rs`) → the frontend version badge (`App.tsx`). Forget it and the binary **and** the badge silently lag the release (e.g. 0.14.0 shipped reporting `v0.13.0` because `Cargo.toml` stayed at 0.13.0). Tag the release commit too, so `git tag` tracks shipped versions.
