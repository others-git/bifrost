...
- **Mobile-friendly is mandatory for every new UI element, not an afterthought.** Any new control (picker, popover, menu, dialog, inline editor) must have a touch-friendly form on phones: branch on `useViewport().isMobile` and render a **bottom sheet** (full-width, large ≥44px tap targets, scrollable) instead of a tiny anchored desktop popover; avoid native `<select>`/hover-only affordances in dense card rows (they crowd the layout and truncate content). Build the mobile path in the same change as the desktop one.
